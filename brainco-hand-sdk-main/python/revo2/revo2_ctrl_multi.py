...
# Multiple devices on one bus require an explicit ID list such as [0x7e, 0x7f].
# This example monitors only the device detected on the fixed COM11 port.
...
async def main():
    ...
    (client, detected_slave_id) = await open_modbus_revo2(port_name="COM11")

    # Configure and monitor only the device actually detected on COM11.
    # Do not probe unrelated hard-coded IDs on this single-device connection.
    for slave_id in [detected_slave_id]:
...
async def get_touch_status(client, slave_id):
    ...
    # Reuse the batch result above. Re-reading all five sensors individually
    # can block this loop before it reaches the motor command.
        ...
        try:
            ...
            if status.is_idle():
                if is_positions_open(status):
                    # When fingers are open, execute grip action
                    # Position values: [Thumb, ThumbAux, Index, Middle, Ring, Pinky]
                    logger.info(f"Device {slave_id:02x} sending grip target")
                    await client.set_finger_positions(slave_id, [500, 500, 1000, 1000, 1000, 1000])
                elif is_positions_closed(status):
                    # When fingers are closed, execute open action
                    logger.info(f"Device {slave_id:02x} sending open target")
